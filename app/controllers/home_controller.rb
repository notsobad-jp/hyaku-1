class HomeController < ApplicationController
  skip_before_filter :require_login

  def index
    if current_user
      #Learn: 今日の歌を取得
      last_song = History.who(current_user).where("created_at < ?", Date.today).original.order("created_at DESC").first
      last_song_id = (last_song) ? last_song.song_id : 0
      @songs = Song.order("id ASC").offset(last_song_id).limit(SONGS_PER_DAY).includes(:histories)
      @learn_num_left = SONGS_PER_DAY - History.who(current_user).which_day(Date.today).original.count

      #Review: 今日の復習対象を取得
      @review_num = 0;
      [1,3,7].each do |d|
        targets = History.original.which_day(d.day.ago).who(current_user).includes(:song)
        instance_variable_set("@review_targets_#{d}", targets)
        @review_num += targets.count
      end

      #Exam: 前回のスコア取得
      @score = History.who(current_user).where(:exam_flg => 1).where(:result => 1).count
    else
      @songs = Array.new
      @review_targets_1 = Array.new
      @review_targets_2 = Array.new
      @review_targets_3 = Array.new
      @score = 0
    end
  end
end
